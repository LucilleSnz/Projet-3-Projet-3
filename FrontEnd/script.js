...
function generateButtons(projects) {
    ...
    const uniqueCategoryNames = ["Tous", ...new Set(categoryNames)];

    const buttons = uniqueCategoryNames.map((categoryName) => {
        const button = document.createElement("button");
        button.className = "buttonFilter";
        button.dataset.name = categoryName;
        button.textContent = categoryName;
        return button;
    });
...
function setActiveButton(button, buttons) {
    buttons.forEach(btn => btn.classList.remove("active"));                    // Retire la classe 'active' de tous les boutons
    button.classList.add("active");                                            // Ajoute la classe 'active' au bouton cliqué
}

/************ Synchronisation des images avec suppression des doublons **************/
async function main() {
    ...
    if (localStorage.getItem("user")) {
        ...
        const buttons = generateButtons(allProjects);
        const buttonContainer = document.getElementsByClassName("filters")[0];
        buttonContainer.append(...buttons);

        // Activer le bouton "TOUS" par défaut
        const defaultButton = buttons.find(btn => btn.dataset.name === "Tous");
        setActiveButton(defaultButton, buttons);

        /************* Récupération bouton ***************/
        buttons.forEach(btn => {
            btn.addEventListener("click", async () => {
                const filterName = btn.dataset.name;
                const projects = await searchProjectsByCategoryName(filterName);
                showProjects(projects);
                
                // Mettre à jour l'état actif du bouton cliqué
